...
public class LambdaMain {
   //this is the functional interface, it can only have one abstract method for lambdas to wwork
    @FunctionalInterface
    ...
    public static void main(String[] args){
        ...
        //lambda form of above
        printPeople(people, (Person p) -> {
                return p.gender == Person.Sex.MALE && p.getAge() >= 18 && p.getAge() <= 25;
        });

        printPeopleWithPredicate(people, (Person p) -> {
            return p.gender == Person.Sex.MALE && p.getAge() >= 18 && p.getAge() <= 25;
        });
    }
}
